...
class Node:
	...
	self.pi.set_servo_pulsewidth(4, HOME_WIDTH)
		...
		for i in range(MIN_WIDTH, MAX_WIDTH, step):
                    rospy.logwarn("i = %d",i)
                    self.pi.set_servo_pulsewidth(4, i)
                    time.sleep(0.1)
                self.pi.set_servo_pulsewidth(4, MIN_WIDTH)
                time.sleep(3)
        """
	...
	offset = 1500
        ...
        self.pi.set_servo_pulsewidth(4, int(target))
